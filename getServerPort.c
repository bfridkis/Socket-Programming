...
void getServerPort(char* serverPort, char* cl_arg1, char* cl_arg2)
{
	...
	if(cl_arg1 != NULL && cl_arg2 != NULL)
	{
		strcpy(serverPort, cl_arg2);
	}
	else
	{
	...
	{
		printf("Enter server port number. (Must be integer between 3000 and 65535)\n--> ");
		fgets(serverPort, 7, stdin);
		bufferClear(serverPort, bufferDeplete);
		//Remove trailing '\n'
		serverPort[strlen(serverPort) - 1] = '\0';
